...
class _RecurringDetailModalState extends ConsumerState<RecurringDetailModal> {
  ...
  Widget _buildDailyControls(
      BuildContext context, DateTime currentDate, TransactionDao dao) {
    final amount = (widget.movement.paymentAmounts?.isNotEmpty == true)
        ? widget.movement.paymentAmounts![0]
        : 0.0;
  ...
  Widget _buildWeeklyControls(
      BuildContext context, DateTime currentDate, TransactionDao dao) {
    final amount = (widget.movement.paymentAmounts?.isNotEmpty == true)
        ? widget.movement.paymentAmounts![0]
        : 0.0;
    final configDay = (widget.movement.paymentDays?.isNotEmpty == true)
    ...
    return _PaymentButton(
      label:
          "Semana del ${monday.day} ${DateFormat('MMM', 'es').format(monday)}",
    ...
    return Column(
      children: [
        _PaymentButton(
          ...
          customCheck: dao
              .countPayments(
                  recurringId: widget.movement.id,
                  start: startOfMonth,
                  end: endOfMonth)
              .then((count) => count >= 1),
        ),
  ...
  void _confirmDelete() {
    showDialog(
        context:
            context, // Usa el context del State, que es seguro tras el check de 'mounted'.
        barrierDismissible: false,
...
class _PaymentButtonState extends State<_PaymentButton> {
  ...
  Widget build(BuildContext context) {
    ...
    final checkFuture = widget.customCheck ??
        widget.dao.isPaymentMade(
        ...
        if (isPaid) {
          return Container(
            padding: const EdgeInsets.symmetric(
                vertical: 12, horizontal: 16), // Reemplazo de withOpacity
            decoration: BoxDecoration(
              ...
              onPressed: () async {
                ...
                if (amount > 0) {
                  final newTx = FinancialTransaction()
                    ...
                    // ✅ CORRECCIÓN: Usamos dateExpected para asignar al período correcto
                    // Esto asegura que el pago del "15" afecte el segundo lapso (15-último),
                    // no el primer lapso (1-15), independientemente del momento real del cobro.
                    ..date = widget.dateExpected
                    ..type = TransactionType.income
                    // ✅ CORRECCIÓN: Marcar como recurrente para que no se cuente como ingreso extra
                    ..isRecurring = true
                    ..categoryName = widget.parentIncome.title
                    ..categoryIconCode =
                        FontAwesomeIcons.moneyBillWave.codePoint
                    ..colorValue =
                        0xFF4CAF50 // Equivalente a Colors.green.value, para evitar el lint de deprecación.
