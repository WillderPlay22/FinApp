...
class _SavingDetailModalState extends ConsumerState<SavingDetailModal> {
      ...
      builder: (context, snapshot) {
        ...
        return DraggableScrollableSheet(
          ...
          builder: (context, scrollController) => Container(
            clipBehavior: Clip
                .hardEdge, // Para que el confeti no se salga si usamos overflow
            ...
            child: Stack(
              children: [
                ListView(
                  // Cambiado a ListView para permitir scroll con el historial
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        Icon(getIconFromCode(saving.iconCode),
                            size: 50, color: Color(saving.colorValue)),
                        IconButton(
                      ...
                      children: [
                        Column(
                          children: [
                            ...
                            const Text("Ahorrado Actualmente",
                                style: TextStyle(
                        ...
                        if (saving.currentAmount > 0) ...[
                          ...
                          ElevatedButton(
                            onPressed: () =>
                                _showUseSavingModal(context, saving),
                    ...
                    if (isGoal) ...[
                      ...
                      const Gap(5),
                      Text("${(progress * 100).toStringAsFixed(1)}% completado",
                    ...
                    // HISTORIAL
                    const Gap(20),
                    const Text("Historial de Ahorros",
                        style: TextStyle(
      ...
      builder: (context, snapshot) {
        if (!snapshot.hasData || snapshot.data!.isEmpty) {
          return const Padding(
            ...
            child: Center(
                child: Text("Aún no hay depósitos.",
  ...
  void _showEditDialog(BuildContext context, Saving saving) {
    ...
    final targetCtrl =
        TextEditingController(text: saving.targetAmount?.toString() ?? "");
              ...
              children: [
                ...
                TextButton.icon(
                  onPressed: () {
                    ...
                    _deleteSaving(
                        context, saving.id); // Llamar confirmación borrar
                  },
                  icon: const Icon(Icons.delete, color: Colors.red),
                  label: const Text("Eliminar Ahorro",
          ...
          children: [
            ...
            if (saving.method == SavingMethod.percentage)
              const Padding(
                ...
                child: Text("Calculado según proyección actual",
          ...
          await isar.writeTxn(() async {
            ...
            final withdrawal = FinancialTransaction()
              ..type = TransactionType
                  .expense // Se marca como egreso interno del ahorro
  ...
  void _deleteSaving(BuildContext context, int id) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
            "Se eliminará el plan, pero las transacciones pasadas quedarán en el historial general."),
        actions: [
          TextButton(
              onPressed: () => Navigator.pop(ctx),
              child: const Text("Cancelar")),
          TextButton(
            onPressed: () async {
              ...
              await isar.writeTxn(() async {
                ...
                final txs = await isar.financialTransactions
                    .filter()
                    .relatedSaving((q) => q.idEqualTo(id))
                    .findAll();
                for (var tx in txs) {
                  tx.relatedSaving.reset(); // Romper el vínculo con el ahorro
                  await isar.financialTransactions
                      .put(tx); // Guardar la transacción actualizada
                }
    ...
    for (var p in particles) {
      ...
      if (dy > -20 && dy < size.height + 20) {
        paint.color = p.color.withValues(
            alpha: (1.0 - progress).clamp(0.0, 1.0)); // Desvanecer al final
